...
void loop() { 
  ...
  if(millis() - lastTime >= 50){
    if (esp8266.available()) {
      ...
      sendCommand(Dir,1,"SEND OK");
      Serial.println("AT comment done");
    }
    lastTime = millis();
    }
  }
...
//連線至AP 
  if(wifi.joinAP(SSID, PASSWORD))
    ...
    Serial.println("Connect AP Failed");
    FailedCount++;
  }
...
void Disconnect(){
    sendCommand("AT+CIPSEND=0,"+ Srtinglength,1,">"); //傳送空白行給server
    sendCommand("",1,"SEND OK");
}
